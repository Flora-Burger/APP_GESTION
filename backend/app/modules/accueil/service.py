...
class AccueilService:
    ...
    def _stats_ordinateurs(self) -> StatsModuleTableauBord:
        ordinateurs = OrdinateurRepository(self.session).obtenir_tous()
        en_service = 0
        indisponible = 0

        for ordinateur in ordinateurs:
            if ordinateur.statut == StatutOrdinateur.OK.value:
                en_service += 1
            else:
                indisponible += 1
    ...
    def _stats_imprimantes(self) -> StatsModuleTableauBord:
        imprimantes = ImprimanteRepository(self.session).obtenir_tous()
        en_service = 0
        indisponible = 0

        for imprimante in imprimantes:
            if imprimante.statut == StatutImprimante.OK.value:
                en_service += 1
            else:
                indisponible += 1

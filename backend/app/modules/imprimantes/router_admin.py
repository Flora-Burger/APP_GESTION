...
def partial_liste_admin_imprimantes(
    ...
    return templates.TemplateResponse(
        ...
        contexte_template(request, imprimantes=imprimantes),
    )
...
def creer_imprimante_admin(
    ...
    compteur_initial: int = Form(default=0),
    service: ImprimanteService = Depends(_obtenir_service),
):
    """Cree une imprimante."""
    redirection = verifier_admin_web(request)
    ...
    try:
        donnees = ImprimanteCreate(
            ...
            compteur_initial=max(0, compteur_initial),
        )
...
def modifier_imprimante_admin(
    ...
    try:
        ...
        service.modifier(imprimante_id, donnees)
        ...
        erreurs_connues = {"nom_existant", "imprimante_introuvable"}
